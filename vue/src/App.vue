<script setup>
  ...
  onMounted(async () => {
    ...
    const sseUrl = 'http://localhost:4000/api/events'
    eventSource = new EventSource(sseUrl)
    eventSource.onmessage = (msg) => {
      const notif = JSON.parse(msg.data)
      if (notif.type === 'do_fetch') receberSlides()
    }
  })

  const expirou = (slide) => {
    return (new Date(slide.expiracao) < new Date()) 
  }
  ...
  const temporizaSlide = () => {
    ...
    if (expirou(slideAtual.value)) index.value = (index.value + 1) % slides.value.length // caso por coincidencia do destino o negocio expire apos o mounted
    timerId = setTimeout(() => {
